...
G.APItems = {
	...
	[G.AP.id_offset + 1] = "b_red",
	[G.AP.id_offset + 2] = "b_blue",
	[G.AP.id_offset + 3] = "b_yellow",
	[G.AP.id_offset + 4] = "b_green",
	[G.AP.id_offset + 5] = "b_black",
	[G.AP.id_offset + 6] = "b_magic",
	[G.AP.id_offset + 7] = "b_nebula",
	[G.AP.id_offset + 8] = "b_ghost",
	[G.AP.id_offset + 9] = "b_abandoned",
	[G.AP.id_offset + 10] = "b_checkered",
	[G.AP.id_offset + 11] = "b_zodiac",
	[G.AP.id_offset + 12] = "b_painted",
...
function IsVanillaItem(key)
	...
	if string.find(key, "^c_") then
		local c_whitelist = { -- Tarots
			...
			"c_soul",
			"c_black_hole",
			"c_base", -- not vanilla but we need these to pass the vanilla check
			"c_rand_ap_tarot",
			"c_rand_ap_planet",
			"c_rand_ap_spectral",
		}
	...
	if string.find(key, "^p_") then
		local p_whitelist = { -- Arcana
			...
			"p_celestial_jumbo_2",
			"p_celestial_mega_1",
			"p_celestial_mega_2", -- Spectral
			"p_spectral_normal_1",
			"p_spectral_normal_2",
			"p_spectral_jumbo_1",
			"p_spectral_mega_1", -- Standard
			"p_standard_normal_1",
			"p_standard_normal_2",
			"p_standard_normal_3",
			"p_standard_normal_4",
...
function APConnect()
				...
				elseif item_id >= 300 and item_id < 330 then
					...
					elseif item_id == 310 then
						...
						if
							...
							and not G.PROFILES[G.AP.profile_Id]["received_indeces"][item.index]
						then
							G.PROFILES[G.AP.profile_Id]["received_indeces"][item.index] = true
	...
	function on_data_package_changed(data_package) end
	...
	function on_bounced(bounce)
		...
		if bounce ~= nil and bounce.tags and tableContains(bounce.tags, "DeathLink") and bounce.data then
			if
				G.AP.LAST_DEATH_LINK_TIME ~= nil
				and tostring(G.AP.LAST_DEATH_LINK_TIME) == tostring(bounce.data.time)
			then
	...
	function on_retrieved(map, keys, extra)
		...
		for _, key in ipairs(keys) do
			if
				key == "balatro_deck_wins" .. tostring(G.AP.player_id) .. "_" .. tostring(G.AP.team_id)
				and type(map[key]) == "table"
			then
				G.E_MANAGER:add_event(Event({
					blocking = false,
					blockable = false,
			...
			if
				key == "balatro_joker_wins" .. tostring(G.AP.player_id) .. "_" .. tostring(G.AP.team_id)
				and type(map[key]) == "table"
			then
				G.E_MANAGER:add_event(Event({
					blocking = false,
			...
			if isAPProfileLoaded() then
				if
					key == "balatro_current_run" .. tostring(G.AP.player_id) .. "_" .. tostring(G.AP.team_id)
					and type(map[key]) == "string"
				then
...
G.AP.update_hints = function()
	...
	if #G.E_MANAGER.queues.ap_hints ~= 0 then
	...
	if isAPProfileLoaded() then
		if e.config.func then
			...
			if G.SAVED_GAME and G.SAVED_GAME.GAME then
				if
					G.SAVED_GAME.GAME.ap_seed == G.APClient:get_seed()
					and G.SAVED_GAME.GAME.ap_jokers_removed == AreJokersRemoved()
					and G.SAVED_GAME.GAME.ap_consums_removed == AreConsumablesRemoved()
					and G.SAVED_GAME.GAME.ap_modded_items == G.AP.this_mod.config.modded
				then
					_can_continue = true
				end
			end
...
G.AP.server_save_decks = function()
	if G.PROFILES[G.SETTINGS.profile].deck_usage then
		G.APClient:Set(
			"balatro_deck_wins" .. tostring(G.AP.player_id) .. "_" .. tostring(G.AP.team_id),
			{},
			false,
			{ { "replace", G.PROFILES[G.SETTINGS.profile].deck_usage } }
		)
	end
end
...
G.AP.server_save_jokers = function()
	if G.PROFILES[G.SETTINGS.profile].joker_usage then
		G.APClient:Set(
			"balatro_joker_wins" .. tostring(G.AP.player_id) .. "_" .. tostring(G.AP.team_id),
			{},
			false,
			{ { "replace", G.PROFILES[G.SETTINGS.profile].joker_usage } }
		)
	end
end
...
G.AP.server_save_run = function(data)
	...
	G.APClient:Set(
		"balatro_current_run" .. tostring(G.AP.player_id) .. "_" .. tostring(G.AP.team_id),
		{},
		false,
		{ { "replace", compressed_save } }
	)
end
