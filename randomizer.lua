...
G.FUNCS.APDisconnect = function(sanity_check)
	...
	if sanity_check then
		if G.SETTINGS.profile == G.AP.profile_Id then
...
function Game:init_game_object()
	...
	if isAPProfileLoaded() then
		...
		init_game_object.interest_cap = init_game_object.interest_cap + (ap_interest * 5 or 0)
...
function G.UIDEF.speech_bubble(text_key, loc_vars)
	if
		isAPProfileLoaded()
		and (G.AP.death_link_cause and G.AP.death_link_cause ~= "unknown" and loc_vars and loc_vars.quip)
	then
...
function Game:update_game_over(dt)
	...
	if
		...
		and not G.AP.game_over_by_deathlink
	then
	...
	if G and G.STAGES and G.STAGE == G.STAGES.MAIN_MENU and G.AP.status_text then
		...
		if G.APClient ~= nil then
			if G.APClient:get_state() == AP.State.SLOT_CONNECTED then
				status_string = string.gsub(
					G.AP.this_mod.config.connection_status ~= 3 and localize("k_ap_connected")
						or localize("k_ap_connected_no_ip"),
					"#3#",
					tostring(G.AP.APSlot)
				)
...
G.FUNCS.AP_unlock_item = function(item, notify)
	...
	if
		not (
			item.set == "Back"
			and tonumber(G.AP.slot_data.stake_unlock_mode) == G.AP.stake_unlock_modes.stake_as_item_per_deck
		)
	then
		if notify then
...
function Game:init_item_prototypes()
	...
	if isAPProfileLoaded() then
		...
		for k, v in pairs(G.localization.descriptions.Back) do
			v.unlock_parsed = {}
			local loc_target = k == "b_challenge" and G.localization.descriptions.Other.ap_locked_Deck_c.text_parsed
		...
		for k, v in pairs(self.P_CENTERS) do
			...
			elseif string.find(k, "^b_") then
				...
				if
					not G.PROFILES[G.AP.profile_Id].deck_usage
					or type(G.PROFILES[G.AP.profile_Id].deck_usage) ~= "table"
				then
		...
		if G.AP.slot_data.required_stake == nil then
			...
			for i = 1, #G.AP.slot_data.included_stakes, 1 do
				G.AP.slot_data.required_stake =
					math.max(G.AP.slot_data.required_stake, G.AP.slot_data.included_stakes[i])
			end
		end
		...
		while ci <= #G.CHALLENGES do
			...
			if not IsVanillaItem(G.CHALLENGES[ci].key) then
				...
				-- init save data
				G.PROFILES[G.SETTINGS.profile].challenge_unlocks[G.CHALLENGES[ci].key] = G.PROFILES[G.SETTINGS.profile].challenge_unlocks[G.CHALLENGES[ci].key]
					or false
		...
		for k, v in pairs(self.P_CENTERS) do
			if
				not v.discovered
				and (string.find(k, "^j_") or string.find(k, "^b_") or string.find(k, "^e_") or string.find(k, "^c_") or string.find(
					k,
					"^p_"
				) or string.find(k, "^v_"))
				and meta.discovered[k]
			then
				v.discovered = true
			end
		end
...
function get_next_voucher_key(_from_tag)
	...
	if isAPProfileLoaded() then
		if
			G.P_LOCKED[get_next_voucher]
			or get_next_voucher == "v_rand_ap_item"
			or get_next_voucher == "UNAVAILABLE"
		then
			return nil
		end
	end
...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
	...
	local create_card =
		create_cardRef(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
...
function CardArea:emplace(card, location, stay_flipped)
	...
	if
		isAPProfileLoaded()
		and (
			((center.unlocked == false and not card.bypass_lock) or center.key == "j_rand_fallback")
			and (
				(G.your_collection ~= nil and tableContains(G.your_collection, self) == false)
				or G.your_collection == nil
			)
		)
	then
...
function G.AP.swap_lock_state(_removed, _types)
	if not _types then
		_types = { "Tarot", "Planet", "Spectral" }
	end
	if type(_types) == "string" then
		_types = { _types }
	end
...
function G.AP.update_ap_debuff(card, juice)
	...
	if IsCardRemoved(center) then
		return nil
	end
	if card.bypass_lock then
		return nil
	end

	if center.ap_unlocked then
		if card.debuff and juice then
			card:juice_up(0.1, 0.06)
		end
...
function Card:set_ability(center, initial, delay_sprites)
	...
	if isAPProfileLoaded() then
		if
			((self.debuff and self.config.center.ap_unlocked == false) or self.config.center.unlocked == false)
			and center.ap_id
			and G.AP.hints
		then
			local _hint
			...
			if _hint then
				if not G.AP.hint_locations[_hint.location] then
					if not G.E_MANAGER.queues.ap_hints then
						G.E_MANAGER.queues.ap_hints = {}
					end
				...
				if not hint.found then
					if self.P_CENTERS[self.APItems[hint.item]] then
						...
						if center.set == "Tarot" or center.set == "Planet" or center.set == "Spectral" then
							if not center.ap_og_set then
								center.ap_og_set = center.set
							end
							center.set = "Joker"
						end
					end
	...
	set_ability = function(self, card, initial, delay_sprites)
		G.E_MANAGER:add_event(Event({
			...
			func = function()
				...
				if
					not (G.your_collection and tableContains(G.your_collection, card.area))
					and card.ability.extra.id == 0
				then
		...
		G.E_MANAGER:add_event(Event({
			...
			func = function()
				...
				if G.APClient ~= nil and tableContains(G.APClient.missing_locations, card.ability.extra.id) then
					-- the gimmick
					if
						G.AP.location_id_to_item_name[card.ability.extra.id]
						and G.AP.location_id_to_item_name[card.ability.extra.id].player_name
						and G.AP.location_id_to_item_name[card.ability.extra.id].player_name == G.AP.APSlot
					then
					...
					G.E_MANAGER:add_event(Event({
						...
						func = function()
							...
							elseif G.AP.Spectral.item.type == "center" then
								if
									G.AP.Spectral.item.center.set == "Joker"
								...
								elseif
									tableContains({ "Tarot", "Planet", "Spectral" }, G.AP.Spectral.item.center.set)
...
G.AP.ap_item_loc_vars = function(self, info_queue, card)
	...
	elseif tableContains(G.APClient.missing_locations, card.ability.extra.id) then
		...
		if
			(set == "Voucher" and item_names == 2)
			or (set ~= "Voucher" and item_names == 3)
			or key == "c_rand_ap_spectral"
		then
			return {}
		end
		...
		if
			G.AP.location_id_to_item_name[card.ability.extra.id]
			and G.AP.location_id_to_item_name[card.ability.extra.id].item_name
		then
			...
			if G.AP.location_id_to_item_name[card.ability.extra.id].game == "Balatro" then
				_item_name, _desc, _info_queue = G.AP.localize_name(
					G.AP.location_id_to_item_name[card.ability.extra.id].item_id,
			...
			if not _skip_name then
				if #_item_name <= 24 then -- use short names as the voucher name
					...
					G.localization.descriptions[set][key .. "_location"].name_parsed =
						G.localization.descriptions[set][key].name_parsed
			...
			G.E_MANAGER:add_event(Event({
				...
				func = function()
					...
					cards[i].children.center:set_sprite_pos({
						x = cards[i].config.center.set == "Planet" and 7
							or cards[i].config.center.set == "Tarot" and 6
							or 5,
						y = 2,
					})
...
function get_shop_location(_pool_length)
	if G.AP.slot_data["stake" .. tostring(G.P_CENTER_POOLS.Stake[G.GAME.stake].stake_level) .. "_shop_locations"] then
		local valid_locations = {}
		local all_locations =
			G.AP.slot_data["stake" .. tostring(G.P_CENTER_POOLS.Stake[G.GAME.stake].stake_level) .. "_shop_locations"]

		-- return existing check if its valid
		if G.GAME.current_shop_check ~= nil then
			if
				tableContains(all_locations, G.GAME.current_shop_check.id)
				and tableContains(G.APClient.missing_locations, G.GAME.current_shop_check.id)
				and G.GAME.current_shop_check.ante == G.GAME.round_resets.ante
			then
		...
		if #valid_locations ~= 0 then
			...
			G.AP.log(
				"Returning Shop Location "
					.. tostring(_check_data.id)
					.. " with price of $"
...
function get_unlocked_jokers()
	...
	if G.P_CENTERS then
		for k, v in pairs(G.P_CENTERS) do
			if
				string.find(tostring(k), "^j_")
				and k ~= "j_rand_fallback"
				and v.ap_unlocked == true
				and not v.modded
			then
				count = count + 1
			end
...
function sendLocationCleared(id)
	...
	if G.APClient ~= nil and G.APClient:get_state() == AP.State.SLOT_CONNECTED then
		if tableContains(G.APClient.missing_locations, id) then
			...
			if
				(
					G.AP.location_id_to_item_name[id]
					and G.AP.location_id_to_item_name[id].player_name
					and G.AP.location_id_to_item_name[id].player_name ~= G.AP.APSlot
				) or G.AP.location_id_to_item_name[id] == nil
			then
...
function SMODS.Center.inject(self)
	if isAPProfileLoaded() then
		if IsVanillaItem(self.key) or (G.AP.this_mod.config.modded ~= 1 and not string.find(self.key, "^b_")) then
...
function AreJokersRemoved()
	if G.AP.this_mod.config.jokers == 3 or (G.AP.this_mod.config.jokers == 2 and G.AP.slot_data.remove_jokers) then
		return true
	end
...
function AreConsumablesRemoved()
	if
		G.AP.this_mod.config.consumables == 3
		or (G.AP.this_mod.config.consumables == 2 and G.AP.slot_data.remove_consumables)
	then
		return true
	end
...
function IsDeathlinkOn()
	if
		G.AP.this_mod.config.deathlink == 3
		or (G.AP.this_mod.config.deathlink == 2 and G.AP.slot_data and G.AP.slot_data.deathlink)
	then
		return true
	end
...
function G.AP.render_ap_debuff(card)
	...
	if card.T.w / card.T.h ~= G.CARD_W / G.CARD_H then
		sticker_type = 2
	end
